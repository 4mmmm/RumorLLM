...
import random
from myutils import preprocess_text
...
class FakeNewsDataset(Dataset):
    def __init__(self, data):
        self.text=data['text']#tokenizer直接放在这，到时候返回mask和ids等
        self.label=data['label']#1是fake，0是real
    ...
    def __getitem__(self, idx):
        ...
        encoded_pair = self.tokenizer.encode_plus(preprocess_text(self.text[idx]),
...
#以下是加入DA后的数据
# train_set=train_set+augmentation_dataset#简单DA，加强的数据全部放在训练集
...
if is_augmentation:
    train_loader=DataLoader(train_set+augmentation_dataset,batch_size=batch_size,shuffle=True)
else:
...
class BertClassify(nn.Module):
  def __init__(self):
    ...
    #     param.requires_grad = False#屏蔽更新权重，不然炸显存
    self.linear1 = nn.Linear(self.bert_dim, self.hidden_dim) # 直接用cls向量接全连接层分类
  ...
  def forward(self, input_ids, attention_mask, token_type_ids):
    ...
    sentence_embedding = torch.mean(sentence_embedding, dim=1).squeeze()  # 压缩取整体句子embedding
...
for epoch in range(epochs):
    ...
    for i, (train_input_ids, train_attention_mask,train_type_ids, train_labels) in enumerate(train_loader):
        train_input_ids, train_attention_mask,train_type_ids, train_labels = to_var(train_input_ids), to_var(train_attention_mask), to_var(train_type_ids), to_var(train_labels)
    ...
    for i, (validate_input_ids, validate_attention_mask,validate_type_ids, validate_labels) in enumerate(validate_loader):
        validate_input_ids, validate_attention_mask,validate_type_ids, validate_labels = to_var(validate_input_ids), to_var(validate_attention_mask), to_var(validate_type_ids), to_var(validate_labels)

        validate_outputs = model(validate_input_ids, validate_attention_mask, validate_type_ids)
    ...
    print ('Epoch [%d/%d],  Loss: %.4f, Class Loss: %.4f, Train_Acc: %.4f,  Validate_Acc: %.4f.'
            % (
            epoch + 1, epochs,  np.mean(cost_vector), np.mean(class_cost_vector),
    ...
    if validate_acc > best_validate_acc:
        ...
        best_validate_dir = output_file +str(time.time())+'_epoch' + str(epoch + 1) + '.pkl'
...
test_pred = []
test_true = []
for i, (test_input_ids, test_attention_mask,test_type_ids, test_labels) in enumerate(test_loader):
    test_input_ids, test_attention_mask,test_type_ids, test_labels = to_var(test_input_ids), to_var(test_attention_mask), to_var(test_type_ids), to_var(test_labels)
